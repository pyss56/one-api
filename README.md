...
25. 支持**渠道请求频率限制**，可为每个渠道单独设置「时间窗口内的最大请求次数」，超出后自动跳过该渠道改用其他渠道，[详见此处](#渠道请求频率限制)。
26. 支持**渠道自动禁用**：可自定义命中哪些上游报错时直接停用渠道（余额耗尽、密钥失效等），并支持在单个渠道上关闭自动禁用。
...
可以为每个渠道单独设置请求频率上限，避免打满上游的限流额度而被返回 429。在渠道编辑页配置两个字段：

| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| 请求次数上限 | `0` | 时间窗口内允许的最大请求次数，**`0` 表示不限制** |
| 时间窗口（秒） | `60` | 统计请求次数的时间窗口长度 |

只填「请求次数上限」即为常见的 RPM 限制；把时间窗口改成 `3600` 就变成「每小时 N 次」，无需增加额外字段。

行为说明：

+ 采用**滑动窗口**统计（任意连续的时间窗口内都不超过上限），而不是按整分整点清零的固定窗口。上游基本都按滑动窗口限流，固定窗口在窗口交界处会放行两倍请求量，依然会触发 429。
+ 达到上限的渠道会在本次请求中被**自动跳过**，改用同一优先级内的其他渠道；最高优先级档全部打满时自动**降级**到低优先级档。
+ 所有候选渠道都打满时，该请求会收到 `429` 响应。**这不会禁用任何渠道**，渠道状态完全不受影响。
+ 未配置（上限为 `0`）时的行为与不开启该功能完全一致。
+ 渠道列表在开启「详细信息」后会实时展示各渠道的「已用 / 上限」请求数，每 10 秒自动刷新一次。
+ 建议同时把运营设置中的「失败重试次数」设为 `1~2`（默认为 `0`，即不重试），与本功能形成互补：频率限制负责事前规避，重试负责事后兜底。

注意：计数保存在进程内存中，**多机部署时每个实例独立计数**，实际总上限约为 `实例数 × 配置值`。需要精确限制请部署为单实例。
